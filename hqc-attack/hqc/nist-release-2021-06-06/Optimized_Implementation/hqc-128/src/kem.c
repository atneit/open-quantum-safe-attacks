 ...
 * @returns 0 if decapsulation is successful, -1 otherwise
 */
void crypto_kem_dec_m(uint64_t *m, const unsigned char *ct, const unsigned char *sk) {
    #ifdef VERBOSE
        printf("\n\n\n\n### DECAPS ###");
    #endif
...
int crypto_kem_dec_sk(unsigned char *ss, const unsigned char *ct, const uint8_t* pk, const __m256i *y_256) {
    ...
    // Decryting
    hqc_pke_decrypt_sk(m, u_256, v, y_256);
...
struct timings crypto_kem_dec_timings(unsigned char *ss, const unsigned char *ct, const unsigned char *sk) {
    ...
    // Encrypting m'
    hqc_pke_encrypt_timings(&rv, u2, v2, m, theta, pk);
    rv.t[6] = toc();
